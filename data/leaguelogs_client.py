...
Deliberately NOT using their Market Index (a second dynasty-value
consensus) — FantasyCalc is the sole value anchor for this project by
design, same reason KeepTradeCut was excluded.

Attribution is required by their terms whenever this data is displayed —
see ATTRIBUTION_HTML, rendered in app.py.
"""
...
_PLAYERS_TTL_SECONDS = 24 * 60 * 60  # LeagueLogs refreshes players daily
...
def get_espn_id(sleeper_player_id: str) -> str | None:
    """LeagueLogs' own ESPN id sync — a third fallback when Sleeper's and
    FantasyCalc's espn ids are both missing for a player."""
...
def get_player_blurb(sleeper_player_id: str) -> dict | None:
    """1-3 sentence LLM-rewritten status note (tags: injury/transaction/
    depth-chart/returning/inactive), or None if there's no recent material."""
